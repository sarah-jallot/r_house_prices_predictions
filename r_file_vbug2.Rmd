...
The list of features is extensive: not all regressors will be useful to predict house price. For instance, we expect the variable MSZoning to have much more impact on the price that the variable Heating, as the heating system is something that can be changed, whereas the location is permanent. We will perform different types of analysis to keep only the most relevant variables in our model.
...
# Anova on OverallCond
mod3<-lm(SalePrice ~ OverallCond-1 , data=home)
anova(mod3)
```
...
- We remove factors with massively overrepresented categories: Street, Utilities, RoofMatl, Condition2, Heating, Electrical, Functional, GarageCond.
- Based on Anova tests, we remove other factors to improve model robustness: OverallCond, Exterior1st, Exterior2nd. OverallCond in particular was too specific and weakened our model by creating observations with leverage one.
- For some features, we were not sure wether or not they had an influence, so we test the model with and without them and remove them if they do not improve our score : HeatingQC, SaleCondition, BsmtFinType2, RoofStyle, ExterQual. 
