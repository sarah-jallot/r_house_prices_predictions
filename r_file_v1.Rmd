...
We will start by a linear model, and then add a Lasso penalization as we have a lot of regressors. Using those two models,the variable selection methods will enable us to select the appropriate regressors.  
...
linear_reg=lm(SalePrice~., data=home)
summary(linear_reg)
```
...
- the general aspects of the home (e.g. Condition1Norm, Condition2, BldgType, OverallQual, OverallCond)
...
linear_select_variables_forward = stepAIC(reg0, SalePrice~., trace=T, direction=c("forward"))
...
```{r}
# Both method
linear_select_variables_both = stepAIC(reg0, SalePrice~., trace=TRUE, direction=c("both"))
