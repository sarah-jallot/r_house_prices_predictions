...
Our objective is to predict the prices of residential homes in Ames, Iowa accurately by exploiting an existing dataset to its best. Our key metric will be the RMSE.     
...
The quantitative variables differ in scale and range: prices start from circa 35000 dollars, and can attain several hundred thousand, whereas bedrooms above ground for instance range from one to eight only. This means that we might have to scale our data to make our model more efficient. 
`````{r, echo=TRUE}
summary(home)
```
