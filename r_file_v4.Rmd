...
```{r, include=FALSE}
home_streamlined = home[-c(5, 8, 21, 13, 22, 23, 40, 66, 34, 37, 20, 59, 26, 55, 62, 38, 51 )]
```
...
home_streamlined[c(199, 596, 336, 692, 633, 323,809),1:ncol(home_streamlined)]
min(home_streamlined$SalePrice)
```

```{r}
# Building the dataframe without those outliers 
home_no_outliers = home_streamlined[-c(199, 596, 336, 692, 633, 323,809), ]
```
...
linear_backward_without_outliers <- lm(formula = log(SalePrice) ~ MSSubClass + MSZoning + LotFrontage + 
    LotArea + LandContour + LandSlope + Neighborhood + Condition1 + 
    OverallQual + OverallCond + YearBuilt + YearRemodAdd + ExterCond + 
    Foundation + BsmtExposure + BsmtFinSF1 + TotalBsmtSF + CentralAir + 
    GrLivArea + BsmtFullBath + FullBath + HalfBath + KitchenQual + 
    Fireplaces + GarageType + GarageCars + GarageQual + WoodDeckSF + 
    YrSold, data = home_no_outliers)
```
