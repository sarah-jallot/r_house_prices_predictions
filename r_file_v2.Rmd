...
1 Intution and examples 

As we said previously, we have the intution that some variables are particularly relevant to predict SalePrice, and that others are not. In this part
As our intuition is that the location of the flat has a big impact, we will test this hypothesis.  

Example of a variable that seems to have an impact on SalePrice (MSZoning) vs one that doesn't seem to have an impact (YrSold). 
```{r, echo=TRUE}
par(mfrow=c(1,2))
boxplot(SalePrice ~ MSZoning, data=home,  xlab="MSZoning", ylab="Sale Price", main="Boxplot of Sale Price based on MSZoning")
boxplot(SalePrice ~ YrSold, data=home,  xlab="Year Sold", ylab="Sale Price", main="Boxplot of Sale Price per Year")
```
  
Compared to MSZoning for which the SalePrice distribution differs for each zone, the variable YrSold, corresponding to the year of sale, does not seem to have an impact on the sale price as it seems to be equally distributed over time when we look at the plot of sale price per year. Looking at the boxplot confirms our intuition that the year of sale does not impact the sale price distribution, as the boxes are almost the same irrespectice of the years.  
...
```{r, echo=TRUE}
# Exactracting all numerical variables
nums = unlist(lapply(home, is.numeric))
home_numerical = (home[ , nums])
```

```{r, echo=TRUE}
# Plotting the distribution and the boxplot for each of these variables
par(mfrow=c(1,2))
...
```{r, echo=TRUE}
data.frame(home_numerical_output)
```


```{r, echo=TRUE}
nrow(home_numerical_output)
```
```{r, echo=TRUE}
head(home_numerical_output)
```
...
for (name in names){cat(which(colnames(home)==name),",")}

home_streamlined = home[-c(6 ,9 ,14, 24, 17)]
```
...
# Forward method :
linear_select_variables_forward = stepAIC(full_model, data=home_streamlined, trace=T, direction=c("forward"))
...
```
According to these results, if we decide to run a linear regression, we would choose the model extracted by the bacward method (or the both method as they return the same AIC). For this specific model, let's verify that the postulates are verified.   
...
linear_backward_without_outliers <- lm(formula = sqrt(SalePrice) ~ MSZoning + LotArea + LotShape + 
    ...
    BldgType + HouseStyle + OverallCond + YearBuilt + YearRemodAdd + 
