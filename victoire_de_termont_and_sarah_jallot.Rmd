...
```
The numerical features that are the most correlated with SalePrice are: GrLivArea, GarageArea and GarageCars, 1st & 2ndFlrSF, YearBuilt GarageYrBuilt & YearRemodAdd. Areas and surfaces are all related to square feet, which is a key driver in house sales. The construction or modernisation works are an indicator of overall quality of the housing and the investments that went into it, so it makes sense for them to be correlated to SalePrice. On the contrary, YrSold and BsmtHalfBath are poorly correlated to SalePrice. YrSold is correlated to none of the other features, so it is an irrelevant regressor: the decision to sell a house doesn't have much to do with what drives its price or the price one can sell it at. 
...
```
3. Factor analysis.  
We first investigate level fragmentation within the factors. We uncover three types of factors, as per examples below.  
...
```
Graphically, we see that overall quality ratings have significant impact over the median SalePrice. We run two ANOVA tests (available in the code) to confirm that area and overall quality both individually have a strong effect on SalePrice: we get a p-value of order e-16. Our ANCOVA shows that MSZoning, OverallQual and their interaction are highly significant towards explaining SalePrice. So we will keep these factors.  
```{r, include = FALSE}
# Anova on area
...
x}
```
We noted earlier that many categorical variables could be considered uninformative or redundant. We removed factors with massively overrepresented categories: Street, Utilities, RoofMatl, Condition2, Heating, Electrical, Functional, GarageCond. We also made some regroupments within factors to diminish cardinality, mostly for neighbourhood by creating two new categories: under 20 and 50 sales. Based on Anova tests, we removed other factors to improve model robustness: OverallCond, Exterior1st, Exterior2nd. OverallCond in particular was too specific and weakened our model by creating observations with leverage one. For some features, we were not sure wether or not they had an influence, so we tested the model with and without them and removed them if they were not improving our score : HeatingQC, SaleCondition, BsmtFinType2, RoofStyle, ExterQual. 
```{r}
...
```
After working on our factors, we start by fitting a full linear model to predict log(SalePrice). Running a model with all the variables excluding the ones we just removed, we obtain an adjusted R-squared of 0.91. Many variables could be removed from our model while marginally affecting its efficiency to explain SalePrice. To improve model efficiency, we perform a selection of variables based on forward, backward and both methods. 
```{r, include = FALSE}
...
summary(model1)
```
Fitting the same model without the outliers, we now have an adjusted R-squared of 93% on train data.  
...
```
The postulates are about verified for this model. Residuals are centered. Scale-location is still slightly elliptic, but we accept it. We now validate autocorrelation. Gaussianity still poses problems for tail values. And no observation has significant Cook distance.  
We now perform winsorisation on SalePrice and fit a new model using the same procedure as earlier : we get an adjusted r-squared of 91% on train data.  
```{r, include = FALSE}
old_sale_price = home_streamlined$SalePrice
...
```
We perform the outlier test to confirm our outliers. Without surprise, 199, 336 and 596 are regression outliers. 633 also is. We remove them: our winsorised model has a 93% R-squared on train data once we remove the outliers. By a visual analysis we do not display here but available in the code, we approximately validate our last problematic postulate, Gaussianity.
```{r, include = FALSE}
...
```
RMSE for model 1 is ~24,5K  dollars and RMSE for model 2 is 31,0K dollars. Even though our first model is less robust than the second, its test error is much lower. So this is the one we will retain since it still validates our postulates. Let us analyse its coefficients in detail. 
...
Confidence intervals: as model 1 is mostly impacted by factors, confidence intervals do not help us visualize the proximity among SalePrice and the most important features. However, doing a 95% confidence interval with LotArea, we notice that this variable is not good at explaning SalePrice by itself, as the price can double for a same value of LotArea. 
```{r, fig.width=4, fig.height=3, fig.align="center"}
...
\textbf{V. Discussion}  
Our postulates are now much better validated than in the full model, with twice as less features (34 instead of 67). Plotting our errors as a function of SalePrice shows that we perform well under 300K dollars, and badly above with very high residuals for extreme values, which skews the RMSE towards the top (the graph is available in the code). Our average residual error is of 18K for properties under 300K dollars, and 55K for properties above, and as we could infer from this, test residuals skewness is very high at 4.5. A solution would be to fit a model on SalePrices that are above this threshold, another one on those under, and then average their predictions to make our final model. Given that we still use 33 regressors, averaging the predictions of a lasso, a ridge and an elastic net performed on the features of model 1 could have been another solution to improve predictions and model robustness. 
...
\textbf{VI. Bonus}  
You can find in the code a quick try at implementing the first solution (without removing any outliers nor looking at the postulates). It would be smarter to find an evident split inside the features rather than in the output, fit two models along these features and then, hand in the predictions of one model or the other depending on where we are in the split. Additionally, the code bugs because some factors have only one level in one of the datasets (which could indicate a good way to split our data!)
```{r, include = FALSE}
home_streamlined$SalePrice <- old_sale_price
# Removing condition 2 as it created an error in both models
home_streamlined2 <- home_streamlined[-c(13)]
```

```{r, include = FALSE}
# Fitting a model on saleprices under 300,000 dollars
mask1 <- home_streamlined2$SalePrice < 300000
home_under300K <- home_streamlined2[mask1,]
reg1 <- lm(log(SalePrice)~., data = home_under300K)
forward1 <- stepAIC(reg1, trace=0, direction=c("forward"), verbose = FALSE)
backward1 <- stepAIC(reg1,~., trace=0, direction=c("backward"))
both1 <- stepAIC(reg1, trace=0, direction=c("both"))

# Over 300,000 dollars
mask2 <- home_streamlined2$SalePrice >= 300000
home_over300K = home_streamlined2[mask2,]
#reg2 <- lm(log(SalePrice)~., data = home_over300K)
#forward2 <- stepAIC(reg2, trace=0, direction=c("forward"), verbose = FALSE)
#backward2 <- stepAIC(reg2,~., trace=0, direction=c("backward"))
#both2 <- stepAIC(reg2, trace=0, direction=c("both"))

# Selecting the best model among these
extractAIC(forward1)
extractAIC(backward1)
extractAIC(both1)

#extractAIC(forward2)
#extractAIC(backward2)
#extractAIC(both2)

# In both cases, we select the backward model
model_under300K <- backward1
#model_over300K <- backward2

# We now average our predictions
test <- test[-c(13)]
#predictions <- (exp(predict.lm(model_under300K, test)) + exp(predict.lm(model_under300K, test)))/2
#sqrt(mean((test$SalePrice - predictions)^2)) # our test error is high at 34K. 
```