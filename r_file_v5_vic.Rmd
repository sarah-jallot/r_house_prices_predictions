...
```  
We first check there are no missing values in the pre-processed data before launching into the analysis.  
```{r}
...
```
We first set out to describe the output, SalePrice. 
```{r}
par(mfrow=c(1,2))
# SalePrice distribution and skewness 
...
```
log(SalePrice) is now pretty close to a normal distribution, except for extreme values. 
  
```{r, fig.width = 4, fig.height = 4, seq = }
...
```
Observing the factor columns, we determine three repartitions.  
i) Clear surrepresentation of one level versus the others. 
In Street and Utilities, which are binary, this is conspicuous. From this we infer that these factors will be uninfomative in predicting house price while weakening our model: nearly all houses will be in the same category along these factors (and for those who aren't, data is too sparse to generalise well).  
This is also the case for RoofMatl, Heating, BsmtFinType2, Electrical, GarageCond, GarageQual.  
...
This is also the case for Exterior1st, Exterior2nd, BsmtExposure. 
...
This is also the case for HeatingQC, GarageFinish, BsmtFinType1.  
...
```{r}
cols_to_remove = c("Street", "Utilities", "RoofMatl", "Condition2","Heating", "Electrical", "Functional", "GarageCond","OverallCond","Exterior1st", "Exterior2nd", "HeatingQC", "SaleCondition", "BsmtFinType2","RoofStyle", "ExterQual")
for (name in cols_to_remove){cat(which(colnames(home)==name)," ")}
```
...
Confidence interval:  
```{r}
#newgrille=data.frame(test=seq(min(test$LotArea)+1,max(test$LotArea)-1),2) 
#predicgrille=predict.lm(linear_backward_without_outliers,newgrille, interval='confidence',level=0.95) 
# head(predicgrille)

#plot(test$LotArea,test$SalePrice,ylab="SalePrice",xlab="LotArea",pch=20) #abline(regP,col='red')
#matlines(newgrille$LotArea,predicgrille,lty=c(1,2,2),col=c("red","blue","blue")) 
#predicgrille=predict.lm(regP,newgrille, interval='prediction',level=0.95) 
#matlines(newgrille$LotArea,predicgrille,lty=c(1,2,2),col=c("red","green","green"))
```
